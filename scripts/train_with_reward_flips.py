...
with a reward function that flips the label with a given probability.
"""
...
# Reward Function Wrapper with Label Flipping
def trl_reward_fn(completions: List[str], prompts: List[str], label_flip_rate: float, **kwargs) -> List[float]:
    ...
    for i, generated_text in enumerate(completions):
        ...
        # Get the true reward first
        rewards_dict = r1_zero_reward_fn(generated_text, ground_truth)
        true_reward = rewards_dict.get("reward", 0.0)
        
        final_reward = true_reward
        
        # Check if we should flip the label
        if random.random() < label_flip_rate:
            # Flip the reward
            final_reward = 1.0 - true_reward
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label_flip_rate", type=float, default=0.0, help="flip label with some probability from 0 to 1.00")
    ...
    # Unsloth automatically prints the trainable parameters
    ...
    reward_function_with_flip = lambda completions, prompts, **kwargs: trl_reward_fn(completions, prompts, label_flip_rate=args.label_flip_rate, **kwargs)
    ...
    trainer = GRPOTrainer(
        ...
        reward_funcs=[reward_function_with_flip],
        tokenizer=tokenizer,
    )
